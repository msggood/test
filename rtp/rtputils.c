...
int main(void)
{
    hndRtp = (struct _tagStRtpHandle*)RtpCreate((unsigned int)((struct sockaddr_in *)inet_addr("192.168.1.222")),6666, _h264nalu);
    unsigned char buf[4];
//    char nalu_buf=calloc(100000, sizeof(char));
    static char nalu_buf[100000];

    static int last_pos=0,now_pos=0,end_pos;
    static int first=1;
    int nalu_len=0;
    ...
    FILE *pNaluFile = fopen("./sintel.h264", "r");
    ...
    fseek(pNaluFile,0,SEEK_END);
    end_pos=ftell(pNaluFile);
    fseek(pNaluFile,0,SEEK_SET);
    
    while(1){
        now_pos=ftell(pNaluFile); //当前位置
        if ((end_pos-now_pos)<4){
            first=1;
            fseek(pNaluFile,0,SEEK_SET);
        }        
//        printf("end_pos=%d,now_pos=%d\n",end_pos,now_pos);
  
        if(4 == fread(buf, 1, 4, pNaluFile)){  
            if(buf[0]==0 && buf[1]==0 && buf[2] ==0 && buf[3]==1){
                if(first==1){
                    last_pos=ftell(pNaluFile)-4;
                    first=0;
                }else
                {
                    now_pos=ftell(pNaluFile)-4;
                    nalu_len=now_pos-last_pos;
                    fseek(pNaluFile,last_pos,SEEK_SET);
                    fread(nalu_buf,1,nalu_len,pNaluFile);
                    gettimeofday(&now,NULL);
                    mnow = (now.tv_sec*1000 + now.tv_usec/1000);//毫秒
//                    SendNalu264(hndRtp,nalu_buf,nalu_len);
                    RtpSend((unsigned int)hndRtp,nalu_buf,nalu_len,mnow);
                    usleep(28000);
                    last_pos=now_pos;
                    fseek(pNaluFile,4,SEEK_CUR);
                }
            }
            fseek(pNaluFile,-3,SEEK_CUR);
        }else{
            printf("read error!!!!!");
            usleep(40000);
        }
